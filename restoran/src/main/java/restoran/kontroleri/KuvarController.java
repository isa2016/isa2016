...
public class KuvarController {
	...
	public ResponseEntity<List<Porudzbina>> findAllPorudzbine() {
		...
		for (int j = 0; j < porudzbine.size(); j++)
			for (int n = porudzbine.get(j).getHrana().size() - 1; n >= 0; n--) {
				if (!(porudzbine.get(j).getHrana().get(n).getTipJela().toString().equals(k.getTipKuvara().toString()))
						|| !(porudzbine.get(j).getHrana().get(n).getStatusJela().equals(StatusJela.ONHOLD)))
					porudzbine.get(j).getHrana().remove(n);
			}
		for (int n = porudzbine.size() - 1; n >= 0; n--) {
			if (porudzbine.get(n).getHrana().size() == 0)
				porudzbine.remove(n);
		}
		return new ResponseEntity<>(porudzbine, HttpStatus.OK);
	}
	...
	public ResponseEntity<List<Porudzbina>> findAllPorudzbinePriprema() {
		...
		for (int i = 0; i < porudzbine.size(); i++)
			for (int j = porudzbine.get(i).getHrana().size() - 1; j >= 0; j--)
				if (!(porudzbine.get(i).getHrana().get(j).getTipJela().toString().equals(k.getTipKuvara().toString()))
						|| !(porudzbine.get(i).getHrana().get(j).getStatusJela().equals(StatusJela.PREPARATION)))
					porudzbine.get(i).getHrana().remove(j);
		for (int n = porudzbine.size() - 1; n >= 0; n--) {
			if (porudzbine.get(n).getHrana().size() == 0)
				porudzbine.remove(n);
		}
	...
	public void prihvatiPorudzbinu(@PathVariable Long id) {
		...
		for (Jelo j : p.getHrana()) {
			if (j.getTipJela().toString().equals(k.getTipKuvara().toString()))
