...
app.controller('loginRegistrationController', [
		'$scope',
		...
		function($scope, loginRegistrationService, $location) {
			...
			$scope.firstLogin = function() {
				...
				loginRegistrationService.firstLogin(firstLoginId,
						$scope.korisnik).then(function(response) {
					$location.path('login');
				});
			};
			...
			$scope.submitLogout = function() {
				loginRegistrationService.logOut().then(function(response) {
